...
mod tests {
    ...
    fn test_key_for_session_format() {
        ...
        assert_eq!(key, "muon-ssh:session-1:password");
    }
    ...
    fn test_load_all_empty_file() {
        let temp_dir = std::env::temp_dir().join("muon_test_creds_empty");
    ...
    fn test_load_all_with_data() {
        let temp_dir = std::env::temp_dir().join("muon_test_creds_data");
        let _ = std::fs::create_dir_all(&temp_dir);
        let path = temp_dir.join("creds.json");
