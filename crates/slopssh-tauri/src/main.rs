use slopssh_core::config::settings::SettingsManager;
use slopssh_core::session::store::SessionStore;
...
use commands::{
    accept_host_key, archive_create, archive_extract, check_for_updates, clear_tab_state,
    create_folder, create_session, create_snippet, credential_delete, credential_get,
    credential_save, delete_session, delete_snippet, deploy_public_key, detect_editors,
    download_update, generate_key_pair, get_app_version, get_settings, get_version,
    import_ssh_config, import_ssh_config_to_folder, list_local_keys, list_remote_keys,
    list_sessions, list_snippets, list_system_fonts, load_tab_state, local_get_drives,
    local_get_home, local_list_dir, local_mkdir, local_remove, local_rename, local_terminal_close,
    local_terminal_open, local_terminal_resize, local_terminal_write, move_session, open_in_editor,
    ...
    save_settings, save_tab_state, sftp_connect, sftp_disconnect, sftp_download_sudo, sftp_home,
    sftp_list_dir, sftp_mkdir, sftp_read_file, sftp_remove, sftp_rename, sftp_stat,
    sftp_upload_sudo, sftp_write_file, ssh_close_shell, ssh_connect, ssh_disconnect,
    ssh_is_connected, ssh_open_shell, ssh_resize_shell, ssh_write_shell, transfer_cancel,
    transfer_clear_completed, transfer_download, transfer_list, transfer_upload, update_session,
    update_snippet, update_tray_tooltip,
};
use state::AppState;
...
fn bounds_file_path() -> Option<std::path::PathBuf> {
    slopssh_core::config::paths::config_dir()
        .ok()
...
pub fn run() {
    ...
    slopssh_core::logging::init(&settings.log_level);

    let session_store = SessionStore::load().unwrap_or_else(|_| {
        let root = slopssh_core::session::folder::SessionFolder::new("Root");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            app.listen("tauri://file-drop", move |event: tauri::Event| {
                ...
                if let Ok(files) = serde_json::from_str::<Vec<String>>(payload) {
                    for file in files {
                        if file.ends_with(".slopssh") {
                            let _ = app_handle
                                .emit("open-session-file", serde_json::json!({ "path": file }));
                        }
                    }
                }
            });
        ...
        .expect("error while running SlopSSH");
}
