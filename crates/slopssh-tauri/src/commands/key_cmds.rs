...
pub async fn list_local_keys() -> Result<Vec<serde_json::Value>, String> {
    ...
    let keys =
        slopssh_core::ssh::key_manager::KeyManager::list_local_keys().map_err(|e| e.to_string())?;
...
pub async fn list_remote_keys(
    ...
    let keys = slopssh_core::ssh::key_manager::KeyManager::list_remote_keys(&handle)
        .await
...
pub async fn generate_key_pair(
    ...
    let key = slopssh_core::ssh::key_manager::KeyManager::generate_key_pair(
        &algorithm,
        &path,
        passphrase.as_deref(),
    )
...
pub async fn deploy_public_key(
    ...
    slopssh_core::ssh::key_manager::KeyManager::deploy_public_key(&handle, &public_key)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn read_public_key(path: String) -> Result<String, String> {
    ...
    slopssh_core::ssh::key_manager::KeyManager::read_public_key(&path).map_err(|e| e.to_string())
}
