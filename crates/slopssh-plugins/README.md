# SlopSSH Example Plugins

This directory contains example WASM plugins for SlopSSH.
...
cd crates/slopssh-plugins/hello
cargo build --release --target wasm32-unknown-unknown
```

## Installing

Place the compiled `.wasm` files in `~/.config/slopssh/plugins/`:

```bash
mkdir -p ~/.config/slopssh/plugins/
cp target/wasm32-unknown-unknown/release/hello_world.wasm ~/.config/slopssh/plugins/
```

Then restart SlopSSH and enable the plugin in Settings > Plugins.
