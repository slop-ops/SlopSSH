use slopssh_core::snippets::Snippet;

#[tauri::command]
pub async fn list_snippets() -> Result<serde_json::Value, String> {
    tracing::debug!("list_snippets");
    let snippets = slopssh_core::snippets::SnippetManager::load().map_err(|e| e.to_string())?;
...
pub async fn create_snippet(snippet: serde_json::Value) -> Result<String, String> {
    ...
    let mut snippets = slopssh_core::snippets::SnippetManager::load().map_err(|e| e.to_string())?;
    ...
    let id = s.id.clone();
    snippets.push(s);
    slopssh_core::snippets::SnippetManager::save(&snippets).map_err(|e| e.to_string())?;
    Ok(id)
}
