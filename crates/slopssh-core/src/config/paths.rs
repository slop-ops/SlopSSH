...
/// otherwise falls back to the OS-specific config directory (`~/.config/slopssh`).
...
pub fn log_dir() -> anyhow::Result<PathBuf> {
    ...
    let dir = dir.join("slopssh").join("logs");
    if !dir.exists() {
        std::fs::create_dir_all(&dir)?;
    }
    Ok(dir)
...
mod tests {
    ...
    fn test_config_dir_returns_valid_path() {
        ...
        assert!(
            dir.to_string_lossy().contains("slopssh") || dir.to_string_lossy().contains("config")
        );
    }
    ...
    fn test_log_dir_returns_valid_path() {
        ...
        assert!(
            dir.to_string_lossy().contains("slopssh") || dir.to_string_lossy().contains("logs")
        );
    }
